...
function suggestions(array, text, type, icon, out, negatable) {
	for(var v of array) {
		...
		} else if(!text.startsWith("minecraft:") && v.startsWith("minecraft:" + text)) {
			var cutText = text;
	...
	if(negatable) {
		for(var v of array) {
			...
			} else if(!text.startsWith("!minecraft:") && output.startsWith("!minecraft:" + text)) {
				out.push({
					text: output,
					type: type,
					iconHTML: icon
...
function suggestionList(type, value, lastText, negatable) {
	...
	if (type == "option") {
		suggestions(value, lastText, "option", "<i class=\"icon option\">?</i>", out, negatable);
			...
			suggestions(scoreboardSlots, lastText, "objective-slot", "<i class=\"icon slot\">s</i>", out, negatable);
	} else if(type == "objective") {
			suggestions(scoreboardObjectives, lastText, "objective", "<i class=\"icon slot\">s</i>", out, negatable);
			...
			if(testText.split("{").length % 2 == testText.split("}").length % 2) {
				...
				if(testText.indexOf("=") == -1) {
					...
					suggestions(sel, testText, "selector-test", "<i class=\"icon slot\">s</i>", out, negatable);
				} else {
...
module.exports = {
	...
	onDidInsertSuggestion: function(args) {
		var types = ["block","effect","advancement","loottable","particle","sound","enchantment","entity-id","item","recipe","slot","objective-slot","objective"];
