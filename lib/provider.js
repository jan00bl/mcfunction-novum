...
/**
 * 
 * Gets information about the datapack currently worked in
 * 
 * @return {{name:String,path:String}} datapack object if present or null if doesn't
 * 
 */
function getDatapack() {
	...
	for (let i = 0; i < pathSplitted.length; i++) {
		if (pathSplitted[i] == "functions") {
			name = pathSplitted[i - 1];
			isInDatapack = true;
			break;
		}
		pathToPack += pathSplitted[i] + "/";
	}
	pathToPack = pathToPack.substring(0, pathToPack.length - 1);
	return isInDatapack ? {name: name, path: pathToPack} : null;
}

function getCustomEntries(tagPath, extension, prefix = "") {
	let files = [];
	let datapack = getDatapack();

	if(datapack == null) {
		return files;
	}

	let crawler = function rec(directory) {
		...
		fs.readdirSync(directory).forEach(file => {
			...
			if (fs.statSync(absolute).isDirectory()) {
				...
				files.push(prefix + datapack["name"] + ":" + absolute.replace(datapack["path"] + tagPath + "/", "").replace("." + extension, ""));
			}
		});
	}

	crawler(datapack["path"] + tagPath);
...
function suggestionList(type, value, lastText, negatable) {
	...
	switch (type) {
		...
		case "advancement":
			...
			suggestions(advancements, lastText, "advancement", icon("option.svg"), out, negatable);
			break;
		case "biome":
