...
function suggestionsWithMc(array, text, type, icon, out) {
	for(var v of array) {
		if(!text.startsWith("minecraft:") && v.startsWith("minecraft:" + text) || "minecraft".startsWith(text)) {
			out.push({
				text: v,
				type: type,
				iconHTML: icon
...
function suggestionsWithoutMc(v, text, type, icon, out) {
	...
	getSuggestions: function (args) {
		...
		if(! lineText.includes(" ")) {
			...
			if(stop["type"] == "command") out = this.getCommandOption(lastText);
				...
				suggestionsWithMc(blocks, lastText, "block", "<img style=\"width:1.5em; height:1.5em;\" src=\"" + __dirname + "/svgicon/block.svg\">", out);
			} else if(stop["type"] == "effect") {
					suggestionsWithMc(effects, lastText, "effect", "<i class=\"icon effect\">e</i>", out);
			} else if(stop["type"] == "advancement") {
					suggestionsWithMc(advancements, lastText, "advancement", "<i class=\"icon advancement\">a</i>", out);
			} else if(stop["type"] == "biome") {
					suggestionsWithoutMc(biomes, lastText, "biome", "<i class=\"icon biome\">b</i>", out);
			} else if(stop["type"] == "structure") {
					suggestionsWithoutMc(structures, lastText, "structure", "<i class=\"icon structure\">st</i>", out);
			} else if(stop["type"] == "loottable") {
					suggestionsWithMc(loottables, lastText, "loottable", "<i class=\"icon loottable\">l</i>", out);
			} else if(stop["type"] == "sound") {
					suggestionsWithMc(sounds, lastText, "sound", "<i class=\"icon sound\">s</i>", out);
			} else if(stop["type"] == "particle") {
					suggestionsWithMc(particles, lastText, "particle", "<i class=\"icon particle\">p</i>", out);
			} else if(stop["type"] == "enchantment") {
					suggestionsWithMc(enchantments, lastText, "enchantment", "<i class=\"icon enchantment\" ><img style=\"width:1.5em; height:1.5em;\" src=\"" + __dirname + "/svgicon/enchantment.svg\"></i>", out);
			} else if(stop["type"] == "entity-id") {
					suggestionsWithMc(entities, lastText, "entity-id", "<i class=\"icon entity\">e</i>", out);
			} else if(stop["type"] == "item") {
					suggestionsWithMc(items, lastText, "item", "<i class=\"icon item\" ><img style=\"width:1.5em; height:1.5em;\" src=\"" + __dirname + "/svgicon/item.svg\"></i>", out);
					suggestionsWithMc(blocks, lastText, "item", "<i class=\"icon item\" ><img style=\"width:1.5em; height:1.5em;\" src=\"" + __dirname + "/svgicon/item.svg\"></i>", out);
			} else if(stop["type"] == "recipe") {
					suggestionsWithMc(recipes, lastText, "recipe", "<i class=\"icon recipe\" ><img style=\"width:1.5em; height:1.5em;\" src=\"" + __dirname + "/svgicon/recipe.svg\"></i>", out);
			} else if(stop["type"] == "slot") {
					suggestionsWithoutMc(slots["inventory"], lastText, "slot", "<i class=\"icon slot\">s</i>", out);
					sort = false;
			} else if(stop["type"] == "objective-slot") {
				suggestionsWithoutMc(slots["scoreboard"], lastText, "slot", "<i class=\"icon slot\">s</i>", out);
