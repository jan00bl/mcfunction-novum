const versions = ["snapshot","1.16.2"];
...
function setArrayList(jsonName, version) {
	let list = require("./1.16.2/id/" + jsonName).slice();
	...
	while((i + 1) < versions.length) {
		...
		try {
			changeList = require("./" + versions[i] + "/id/" + jsonName);
		} catch (error) {
			i++;
			continue;
		}

		for (const elem of changeList) {
			switch (elem["type"]) {
				case "add":
...
function setObjectList(jsonName, version) {
	let object = JSON.parse(JSON.stringify(require("./1.16.2/id/" + jsonName)));
		...
		if (object.hasOwnProperty(key)) {
			...
			if(Array.isArray(element)) {
				...
				while ((i + 1) < versions.length) {
					...
					try {
						changeList = require("./" + versions[i] + "/id/" + jsonName)[key];
					} catch (error) {
						i++;
						continue;
					}
 ...
 */
function icon(name) {
	if (atom.config.get("mcfunction-novum.showIcons")) {
...
function suggestionList(type, value, lastText, negatable) {
	...
	switch (type) {
		...
		case "structure":
			...
			break;
		case "loottable":
			suggestions(loottables, lastText, "loottable", icon("loottable.svg"), out, negatable);
			break;
		case "sound":
