...
const noItem = require("./id/no_item.json");
const gamerules = require("./id/gamerule.json");
...
module.exports = {
	...
	runCycle: function(args, cycle) {
		...
		for(; i < args.length; ) {
			...
			if ((realStop["type"] == "option" || realStop["type"] == "particle" || realStop["type"] == "gamerule") && realStop["change"] != null && realStop["change"][arg] != null) {
				...
				let cmd = args[i];
				let newCycle = commands["commands"][cmd];
				return {
					pos: cycle.length + 1,
					argPos: args.length + 1,
	...
	onDidInsertSuggestion: function(args) {
		let types = ["attribute-id","block","effect","advancement","loottable","particle","sound","enchantment","entity-id","item","recipe","slot","objective-slot","objective","biome"];
